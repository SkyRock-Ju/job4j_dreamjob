...
public class Sql2oCandidateRepository implements CandidateRepository {
    ...
    public Candidate save(Candidate candidate) {
        try (var connection = sql2o.open()) {
            var sql = """
                      INSERT INTO candidates(title, description, creation_date, file_id)
                      VALUES (:title, :description, :creationDate, :fileId)
                      """;
    ...
    public boolean update(Candidate candidate) {
        try (var connection = sql2o.open()) {
            var sql = """
                    UPDATE candidates
                    SET title = :title, description = :description, creation_date = :creationDate, file_id = :fileId
                    WHERE id = :id
                    """;
